...
def optimize_network_hyperparams(network_name, network_data, n_trials=50, study_name=None, use_optimized=True, seed=777, use_storage=True, storage_path="optimization_history.db", warm_start=True):
    """
    Optimises hyperparameters for a specific network
    
    Args:
 network_name: network name ('visual', 'dorsattn', 'sommot', 'multi')
 network_data: network data 
 n_trials: number of trials
 study_name: study name (optional)
 use_optimised: if True, use optimised version with pre-computed datasets
 seed seed for reproducibility (default 777)
 use_storage: if True, use SQLite persistent storage
 storage_path: SQLite database path (only if use_storage=True)
 warm_start: if True, start from your current configurations
...
def optimize_with_storage(network_name, network_data, n_trials=50, storage_path="optimization_history.db", 
    ...
    try:
        ...
        print(f"Existimg study loaded - {len(study.trials)} trials already completed")
